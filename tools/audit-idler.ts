...
import { MAX_FLOORS } from '../src/sim/data';
import { roomCapacityAt, roomSlotsUsed } from '../src/sim/dungeon';
  ...
  while (!s.over) {
    ...
    for (let f = 0; f < s.dungeon.floors.length; f++)
      for (let r = 0; r < s.dungeon.floors[f]!.rooms.length; r++)
        if (roomSlotsUsed(s.dungeon, f, r) > roomCapacityAt(s.dungeon, f, r)) overCap++;
